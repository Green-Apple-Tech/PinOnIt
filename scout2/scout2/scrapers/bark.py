"""Bark.com pro-profile scraper via ScaleSerp."""
...
def is_bark_profile(url: str) -> bool:
    try:
        p = urlparse(url)
    except Exception:
        return False
    host = (p.hostname or "").lower()
    if "bark.com" not in host and "bark.us" not in host:
        return False
    path = p.path.lower()
    if not path or path == "/":
        return False
    ...
    async with httpx.AsyncClient() as client, PoliteFetcher() as fetcher:
        for niche in niches:
            ...
            for url in links:
                if deadline and deadline.expired():
                    break
                if not is_http_url(url) or not is_bark_profile(url):
                    continue
