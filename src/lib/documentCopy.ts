...
export const SIGN_BY_TEXT_SCOPE_SUMMARY =
  'Sign-by-Text is for single-signature business documents (waivers, NDAs, addendums, estimates, job sign-offs). Not for wills, trusts, powers of attorney, deeds, court papers, or anything that needs a notary or multiple signers.';
...
/** Once-per-account checkbox label. */
export function signByTextAckLabel(maxLabel = '5MB') {
  return `I understand Sign-by-Text is for single-signature business documents only — not wills, trusts, powers of attorney, deeds, court papers, or notarized/multi-signer docs. Uploaded PDFs must be clear and complete (PDF, up to ${maxLabel}).`;
}
